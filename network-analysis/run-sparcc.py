...
run_command(['./SparCC.py', feature_table_fp, '-c', correlations_fp], verbose=True)

bootstrap_dir = os.path.join(output_dir, 'bootstraps/')
os.mkdir(bootstrap_dir)

run_command(["./MakeBootstraps.py", feature_table_fp, "-t", "permutation_#.txt", "-p", bootstrap_dir, "-n", bootstrap_iterations], verbose=True)
...
for i in range(bootstrap_iterations):
    ...
    run_command(['./SparCC.py', perm_feature_table_fp, '-c', perm_correlations_fp], verbose=True)

pvalues_fp = os.path.join(output_dir, "p-value.out")
perm_correlations_pattern = os.path.join(bootstrap_dir, 'perm_cor_#.txt')

run_command(["./PseudoPvals.py", correlations_fp, perm_correlations_pattern, bootstrap_iterations, "-o", pvalues_fp, "-t", "two_sided"], verbose=True)
